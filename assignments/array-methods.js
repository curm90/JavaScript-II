...
// Problem 1
const company = runners.map(runner => {
  return `${runner.first_name} ${runner.last_name} works for ${runner.company_name}`
});
console.log(company);

// Problem 2
const sortedDonations = runners
  .map(runner => runner.donation)
  .sort((a, b) => a < b ? 1 : -1);
console.log(sortedDonations);

// Problem 3
const howManySmallShirts = runners.filter(runner => runner.shirt_size === 'S').length;
console.log(howManySmallShirts);
